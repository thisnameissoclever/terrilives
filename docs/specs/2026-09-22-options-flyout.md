...
The gear is a 44 by 44 button, named "Options" for assistive technology, fixed to the window's top-right corner inside the safe-area insets, on every screen size and in and out of Build. Its panel opens beneath it, at most 240 pixels wide and never taller than the window, and scrolls on its own. Both live outside `#hud`, as `#object-menu` does, because they are fixed to the window rather than part of the sidebar's column.

The gear stacks above the phone Build dock (z-index 2) and below the debug overlay (9) and the right-click flyout (10), at 8. The debug overlay moves 52 pixels down, out of the corner. On a phone in portrait the sidebar's right edge moves 52 pixels in, so the gear never covers the Menu button.
...
The played check drives the gear, the panel and Build from it at 1280 by 800 and 375 by 812, and opens the Traits panel on each. Tests pin the controller's open, close, Escape and outside-press rules; that every moved control is inside the panel and every status line inside the household status; the gear's name, its link to the panel and its hidden start; its fixed position, safe-area insets and stacking; that no compact strip keeps a Light or Build row; and that main.ts registers the panel's Escape before Build's and closes the panel when Build starts.
