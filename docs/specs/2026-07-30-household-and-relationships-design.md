...
Status: **decided, not yet built.** Made without stopping to ask, per the
standing instruction; each carries the alternative rejected and why.
...
## [H1] A stable `SimId`, and it is not the entity index

Every sim gets a `SimId(u32)` from a monotonic counter held as a world resource,
assigned at spawn and never reused.
...
## [H2] The household is content, not something the shell spawns

A new `content/household.toml` lists the sims: name, personality archetype,
starting needs, spawn tile.
...
## [H3] Personality is two multiplier arrays plus a disposition list
...
## [H4] A sim is an advertiser, and that is the real change
...
## [H5] Relationships are one number per ordered pair
