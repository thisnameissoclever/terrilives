...
Every visible atlas sprite is original Muted Line art generated by the Python
and Pillow code in `assets/sprites/gen/`; CI regenerates the atlas and fails on
a diff. The runtime has three stable baked character looks, per-instance tint
and emissive strength, a clock-driven ambient cycle, wall-aware lamp and
television pools, footprint-derived cast shadows, and a deterministic two-pixel
walking lift. The selection ring remains planted and uses a full-emissive pale
outer key so world lighting cannot erase the command target. Reduced motion
disables the ornamental lift and forces the same neutral flat-light mode
exposed by the HUD.
There is no borrowed pack art, live 3D mesh pipeline, external character rig,
imported animation library, or second lighting pass in the shipped alpha.

### Superseded full-art alternative

Everything below this heading is retained as design history, not as the current
roadmap. Muted Line settled the shipped and near-term pipeline on 2026-08-03.
Revisit these 3D options only if Create-a-Sim or cast scale proves the generated
sprite approach cannot meet the customization requirement.
