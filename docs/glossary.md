...
| **activity code** | What a row is doing: 0 none, 1 walking, 2 waiting, 3 exact authored eating, 4 talking, 5 sleeping, 6 at work, 7 generic object use, 8 exact seated reading. Some activities are text-only and deliberately draw no indicator bubble. |
| **visual action code** | Which authored body pose a row draws: 0 none, 1 talk, 2 eat, 3 read. Activity alone never invents a visual action; the compiled interaction contract owns it. |
