...
**What happened:** PR 116 added three Rust tests, and eight mutants that had passed for months were reported as hangs: the two `rect_distance` clamps, `SimRng::from_seed`'s `|` to `^`, two `Needs::as_slice` replacements, two `flood_fill` arithmetic mutants and `ContentPack::catalogue`. Three of the eight are recorded in `docs/mutation-baseline.md` as equivalent, with the arithmetic proof, and two of the others were caught on the run before. Nothing about any of them loops.

**Root cause:** a mutant that no test kills, and a mutant caught only by a late test, runs the whole workspace suite, so its test phase costs a full suite. CI capped a mutant's test run at a fixed 60 seconds, chosen when the suite took about two seconds. The suite had grown to about 57 seconds on the runner, so every such mutant finished a few seconds under the cap, and the next tests added pushed eight of them past it. The hang gate then accused the wrong code: each of those functions was read for an unbounded loop that was never there.

**The worse half, found while fixing it:** a cap below the suite does not report hangs at all. The unmutated baseline run fails first, cargo-mutants tests no mutants, and it still writes an empty `missed.txt` and an empty `timeout.txt`, which every gate reads as success. On PR 117, six of the eight shards had already crossed that line: their baseline timed out, `total_mutants` was 0, and all six reported green. The mutation sweep had been proving nothing on most shards and nothing said so.

**Prevention rule:** the cap is a multiple of the unmutated run (`--timeout-multiplier 4`), never a constant, so it moves with the suite; the job carries a 90 minute bound, because the multiplier cannot bound the unmutated run itself; and a step fails any shard whose sweep tested zero mutants, because an empty output file is not evidence. A gate that reads a file must also check that the file was written by a run that did something.

**How to verify:** download a shard's `mutants-out-shard-N` artifact and read `outcomes.json`. `total_mutants` must be above zero, a survivor's test phase should sit near the baseline scenario's rather than at the cap, and `timeout.txt` must be empty. The evidence here: PR 117 shard 1 showed 57.1, 57.1 and 59.5 seconds for the three equivalent mutants and 54.5 and 55.9 for the two caught ones, against the 60 second cap; PR 116 showed 60.0 for all of them; PR 117 shards 2 to 7 showed `total_mutants` 0 with a 60.0 second baseline.
