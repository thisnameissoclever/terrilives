...
/// (3, 3), the front door at (6, 3) and `funds` in the bank. Returns the
/// fridge and the coat rack.
...
/// The ECS frees indices of its own, so the next spawn need not be a fresh
/// index; what matters is that no spawn ever takes the sold one.
#[test]
