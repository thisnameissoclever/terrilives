...
fn placement_boundary_rejects_hostile_numbers_before_coercion_in_release() {
    ...
    for invalid in [f64::NAN,f64::INFINITY,f64::NEG_INFINITY,-1.0,0.5,u32::MAX as f64+1.0,1e30] {
        for index in 0..4 {
            let mut args = [0.0,1.0,1.0,f];
            args[index] = invalid;
            let before = handle.save_bytes();
            assert_eq!(handle.placement_preview(args[0],args[1],args[2],args[3])[0],1.0);
    ...
    for f in [4.0,255.0,256.0,u32::MAX as f64] { assert!(!handle.place_object(0.0,1.0,1.0,f)); }
    assert!(handle.object_facing_mask(0.0) > 0);
    assert!(handle.place_object(0.0,1.0,1.0,f),"valid control must enqueue");
...
fn placement_boundary_reports_refusal_separately_from_queue_acceptance() {
    let mut handle = SimHandle::from_lot();
    let f = handle.object_facing(0.0).unwrap() as f64;
    let (x,y) = handle.sim.world().resource::<Content>().0.lot.walls[0];
    let before = handle.save_bytes();
...
fn placement_boundary_preserves_appended_wire_code_and_pending_save_replay() {
    ...
    for y in 0..original.lot_height() {
        for x in 0..original.lot_width() {
            if (x,y) != (0,0) && original.placement_preview(0.0,x as f64,y as f64,f as f64)[0] == 0.0 { chosen = Some((x,y)); break; }
        }
        if chosen.is_some() { break; }
    }
    let (x,y) = chosen.unwrap();
    assert!(original.place_object(0.0,x as f64,y as f64,f as f64));
    assert_eq!(postcard::to_allocvec(&original.sim.world().resource::<CommandQueue>().as_slice()[0]).unwrap(),[7,0,x as u8,y as u8,f as u8]);
    let saved = original.save_bytes();
    let mut restored = SimHandle::new(1,1);
    assert!(restored.load_bytes(&saved));
    original.flush_commands(); restored.flush_commands();
