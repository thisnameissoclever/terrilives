...
describe('reviewed bathroom sink', () => {
  it.each(['', 'NW', 'SW', 'NE'])('registers and picks facing %s', (suffix) => {
    const index = atlas.spriteIndex(`offlineBathroomSink${suffix}`);
    expect(index).toBe(1105 + ['', 'NW', 'SW', 'NE'].indexOf(suffix));
    ...
    const source: PickSource = {
      ...
      ids: () => new Uint32Array([12]),
      sprites: () => new Uint32Array([index]),
      activities: () => new Uint32Array([0]),
    };
    expect(pickSprite(source, 0, -20, 0, 0)).toEqual({ entity: 12, isAgent: false });
