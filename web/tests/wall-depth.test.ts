...
describe('edge wall projection', () => {
        ...
        for (const scale of [1, 1.75, 3]) {
            ...
            for (let n = 0; n < result.count; n++) {
                ...
                if (n < result.floorCount) {
                    expect(Array.from(row.subarray(8, 12))).toEqual([0, 0, 0, 0]);
                    continue;
                }
    ...
    it('leaves legacy cell walls on their original depth contract', () => {
        ...
        for (let n = 0; n < result.count; n++)
            expect(Array.from(result.instances.subarray(n * FLOATS_PER_INSTANCE + 8, n * FLOATS_PER_INSTANCE + 12))).toEqual([0, 0, 0, 0]);
    });
