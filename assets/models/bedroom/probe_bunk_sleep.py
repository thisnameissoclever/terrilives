...
def run(model, output, all_views=False):
    ...
    try:
        ...
        result['renders'] = []
        facings = {'SE':90, 'NW':270, 'SW':0, 'NE':180} if all_views else {'SE':90}
        for facing, degrees in facings.items():
            for frame in (range(1, 5) if all_views else (1,)):
                scene.frame_set(frame)
                ...
                path = output/f'sleep-{facing}-{frame-1}-beauty.png'
                scene.render.filepath = str(path)
                bpy.ops.render.render(write_still=True)
                result['renders'].append({'facing':facing, 'frame':frame-1,
                                          'path':path.name,
                                          'sha256':hashlib.sha256(path.read_bytes()).hexdigest()})
                save()
        assert hashlib.sha256(model.read_bytes()).hexdigest() == before
        assert result['script_sha256'] == hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
        assert result['contact_script_sha256'] == hashlib.sha256((BASE/'bunk_contact.py').read_bytes()).hexdigest()
...
if __name__ == '__main__':
    ...
    if len(args) not in (2, 3) or (len(args) == 3 and args[2] != '--all-views'):
        raise ValueError('Pass saved bunk scene and a new probe directory, optionally --all-views')
    run(Path(args[0]), Path(args[1]), len(args) == 3)
