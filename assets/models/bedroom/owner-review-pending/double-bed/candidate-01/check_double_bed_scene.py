...
def validate():
    ...
    for foot in feet:
        ...
        rail = bpy.data.objects['Side rail -0.815' if foot.location.x < 0 else 'Side rail 0.815']
        witness = overlap_witness(foot, rail, deps)
        assert witness, 'Foot detached from side rail'
        contacts[foot.name] = witness
    for obj in root.children:
        low, high = bounds(obj)
        assert all(low[axis] >= -.98 and high[axis] <= .98 for axis in (0, 1)), 'Part leaves four-tile footprint'
        if 'support' in obj:
    ...
    assert high[1]-low[1] >= 1.7, 'Mattress is too short'
    left = bounds(bpy.data.objects['Pillow -0.415'])
    right = bounds(bpy.data.objects['Pillow 0.415'])
...
if __name__ == '__main__':
    ...
    try:
        ...
        for name, axis, amount, expected in (
            ('Foot -0.79 -0.85', 'z', .08, 'Foot lost floor contact'),
            ('Pillow -0.415', 'z', .15, 'Pillow -0.415 detached from Mattress'),
